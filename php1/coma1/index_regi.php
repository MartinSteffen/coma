...
if (isset($_POST['email'])){
   ...
   *
   */
  // Test, ob alle Pflichtfelder ausgef¸llt wurden
  if ( $_POST['last_name']=='' || $_POST['email']=='' || $_POST['userPassword']==''){
   $strMessage = ' Email, Passwort und Nachname m¸ssen angegeben werden !!! ' ;
   $strMainAssocs['message'] = $strMessage; 
  }
  // Test, ob Passwort mit der Wiederholung ¸bereinstimmt
  else if ( $_POST['userPassword'] !=  $_POST['userPassword2']){
    $strMessage = ' Passwort stimmt nicht mir der Wiederholung ¸berein !!! ' ;
    $strMainAssocs['message'] = $strMessage; 
    }
  // Test, ob die Email g¸ltig ist
  else if (!($_POST['email']!="" && 
	     ereg("^([a-zA-Z0-9\.\_\-]+)@([a-zA-Z0-9\.\-]+\.[A-Za-z][A-Za-z]+)$", $_POST['email']))){
    $strMessage = ' Bitte geben Sie eine g¸ltige Emailadresse ein !!! ' ;
    $strMainAssocs['message'] = $strMessage; 
